...
public class CartMasterLogicService implements ICartMasterLogicService{
    ...
    //Microservce-host mapping
      private static String CART_HOST =  "marketplace-microservice-cart.herokuapp.com";
      private static String ITEM_HOST =  "marketplace-microservice-item.herokuapp.com";
...
//    @Requires
    public CartMasterDTO createMasterCart(CartMasterDTO cart) {
        ...
        WebTarget target = client.target(CART_HOST).path("Cart/webresources/Cart/");
...
//        CartDTO persistedCartDTO = cartPersistance.createCart(cart.getCartEntity());
        if (cart.getCreateitem() != null) {
            for (ItemDTO itemDTO : cart.getCreateitem()) {
                target = client.target( ITEM_HOST ).path("Item/webresources/Item/");
...
//                itemPersistance.updateItem(itemDTO);
    ...
    public void deleteMasterCart(Long id) {
        Client client = ClientBuilder.newClient();
        WebTarget target = client.target( CART_HOST ).path("Cart/webresources/Cart/" + id);
...
//                itemPersistance.updateItem(itemDTO);
        ...
        if (cart.getDeleteitem() != null) {
            for (ItemDTO itemDTO : cart.getDeleteitem()) {
                ...
                target = client.target( ITEM_HOST ).path("Item/webresources/Item/"+itemDTO.getId());
                target.request(MediaType.APPLICATION_JSON).delete();
